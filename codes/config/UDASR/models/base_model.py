...
##                	print('skip-conv_first!')
#                    continue
                else:
                    load_net_clean[k] = v
            # network.load_state_dict(load_net_clean, strict=strict)
#            print(f'load_net_clean:{load_net_clean[params_ema]}')
            if 'params_ema' in load_net_clean:
                #load rrdb weight
                print('model1')
                keyname = 'params_ema'
                load_net_clean_new = OrderedDict()
                for key,value in load_net_clean[keyname].items():
#                    load_net_clean_new[key]=value # use channel3 rrdb weight
                    if key.startswith("conv_first.weight"):#use channel3 rrdb init channel4
                        print('skip rrdb first conv---')
                        print(f'value shape==:{value.shape}')
                        print(f'key==:{key}   type:{type(value)}')
                        zore_weight=torch.zeros((64,1,3,3))
                        new_conv_first_weight=torch.cat((value,zore_weight),dim=1)#shape (64,4,3,3)
#                        print(f'new_conv_first_weight:{new_conv_first_weight.shape}')
                        load_net_clean_new[key]=new_conv_first_weight
                    else:
                        load_net_clean_new[key]=value
#                print("=====================")
#                print(load_net_clean_new)
                network.load_state_dict(load_net_clean_new, strict=strict)
            elif 'params'in load_net_clean:
                print('model2')
                keyname = 'params'
                network.load_state_dict(load_net_clean[keyname], strict=strict)
            else:
                network.load_state_dict(load_net_clean, strict=strict)            
#            ---
...
#            network.load_state_dict(network_state_dict, strict=strict)
